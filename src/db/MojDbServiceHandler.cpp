...
const MojDbServiceHandler::SchemaMethod MojDbServiceHandler::s_privMethods[] = {
	...
	{MojDbServiceDefs::RegisterMediaMethod, (Callback) &MojDbServiceHandler::handleRegisterMedia, MojDbServiceHandler::RegisterMediaSchema},
	{NULL, NULL, NULL} };
...
MojErr MojDbServiceHandler::handleShardKind(MojServiceMessage* msg, MojObject& payload, MojDbReq& req)
{
    ...
    else
    {
        ...
        {
            bool isValid;
            err = m_db.isValidKind(kindStr, isValid);
            MojErrCheck(err);

            if (G_LIKELY(isValid))
            {
                bool isSupported;
                err= m_db.isSupported(shardIdBase64, kindStr, isSupported);
                MojErrCheck(err);

                //ok
                err = writer.boolProp(MojServiceMessage::ReturnValueKey, true);
                MojErrCheck(err);
                err = writer.stringProp("isSupported", (isSupported) ? "true" : "false");
                MojErrCheck(err);
 ...
 * handleRegisterShard
 *
 * Register new external media
 *   @parameter:
 *   @return:
 *     - returnValue:false
 *     - errorText: "Invalid Shard ID"
 ***********************************************************************/
MojErr MojDbServiceHandler::handleRegisterMedia(MojServiceMessage* msg, MojObject& payload, MojDbReq& req)
{
    // check space level
    if( m_db.getSpaceAlert().spaceAlertLevel() == MojDbSpaceAlert::AlertLevelHigh)
       return MojErrDbQuotaExceeded;
...
MojErr MojDbServiceHandler::convert(const MojObject& object, MojDbShardInfo& shardInfo)
{
    MojErr err;
...
bool MojDbServiceHandler::existInCache(const MojString& id)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
...
void MojDbServiceHandler::copyShardCache(std::set<MojString>* shardIdSet)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
