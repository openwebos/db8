...
MojDbSandwichTableTxn::MojDbSandwichTableTxn(BackendDb &txn, const BackendDb::Part &db)
    : m_txn(txn)
    , m_db(db)
...
std::unique_ptr<leveldb::Iterator> MojDbSandwichTableTxn::createIterator()
{
    MojAssert( m_db.Valid());
    return m_db.NewIterator();
...
MojDbSandwichTableTxn &MojDbSandwichEnvTxn::tableTxn(MojDbSandwichEngine::BackendDb::Part &db)
{
    ...
    auto it = m_tableTxns.find(&db);
    if (it != m_tableTxns.end()) return *it->second;
    /*
    ...
    */
    ...
    txn.reset(new MojDbSandwichTableTxn(m_txn, db.ref(m_txn)));
    //txn->begin(db);
...
MojErr MojDbSandwichEnvTxn::commitImpl()
{
    ...
    if (!isReversed()) {
        for(TableTxns::iterator it = m_tableTxns.begin();
                            ...
                            ++it)
        {
            MojErr err = it->second->commitImpl();
