...
MojErr doMigrate(MojObject& confObj)
{
    ...
    {
        LOG_ERROR(MSGID_DB_ERROR, 0, "Database is already converted.)");
        return MojErrNone;
    }
    ...
    {
        LOG_ERROR(MSGID_DB_ERROR, 0, "Database is locked.)");
        return MojErrExists;
    }
    ...
    for (const auto &part : parts)
    {
        ...
        if (!s.ok())
        {
            std::cerr << "Failed to open source database " << (ldbPpath + part).c_str()  << ": " << s.ToString() << std::endl;
            return MojErrDbIO;
        }
    }

    MojString bkPath;
    bkPath.append("/media/cryptofs/data/db8/migration");
    ...
    typedef leveldb::SandwichDB<leveldb::BottomDB> BackendDb;
    {
        ...
        if (!s.ok())
        {
            std::cerr << "Failed to open destination database " << bkPath.data()  << ": " << s.ToString() << std::endl;
            return MojErrDbIO;
        }
